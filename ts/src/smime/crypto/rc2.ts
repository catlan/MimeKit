// RC2 (RFC 2268) in CBC mode.
//
// The MimeKit S/MIME test PKCS#12 files protect their certificate SafeContents
// with pbeWithSHAAnd40BitRC2-CBC (RFC 7292). WebCrypto and @noble have no RC2,
// so this is a userland port of the RFC 2268 cipher. Isomorphic (no node:crypto).
...
/** Expand an RC2 key into 64 16-bit words (RFC 2268 §2), with effective key bits `t1`. */
function expandKey(key: Uint8Array, effectiveBits: number): number[] {
  const t = key.length;
...
function decryptBlock(k: number[], data: Uint8Array, offset: number): Uint8Array {
  ...
  const rrot = (x: number, s: number): number => ((x >>> s) | (x << (16 - s))) & 0xffff;
...
export function rc2CbcDecrypt(key: Uint8Array, effectiveBits: number, iv: Uint8Array, data: Uint8Array): Uint8Array {
  ...
  for (let off = 0; off < data.length; off += 8) {
    const block = decryptBlock(k, data, off);
