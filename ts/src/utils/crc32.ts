/**
 * Port of MimeKit/Utils/Crc32.cs.
 */
...
export class Crc32 {
  ...
  update(valueOrBuffer: Uint8Array | number, offset = 0, count?: number): number {
    if (typeof valueOrBuffer === 'number') {
      this.crc = (crc32Table[(this.crc ^ valueOrBuffer) & 0xff]! ^ (this.crc >>> 8)) >>> 0;
      return this.crc;
    }

    const buffer = valueOrBuffer;
    const length = count ?? buffer.length - offset;
    if (!Number.isInteger(offset) || offset < 0 || offset > buffer.length)
      throw new RangeError(`offset ${offset} out of range [0, ${buffer.length}]`);
    if (!Number.isInteger(length) || length < 0 || length > buffer.length - offset)
      throw new RangeError(`count ${length} out of range [0, ${buffer.length - offset}]`);

    const end = offset + length;
    for (let i = offset; i < end; i++)
      this.crc = (crc32Table[(this.crc ^ buffer[i]!) & 0xff]! ^ (this.crc >>> 8)) >>> 0;
